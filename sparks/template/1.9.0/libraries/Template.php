...
class Template
{
	 ...
	 * @return	void
	 */
	public function build($dir, $view, $data = array(), $return = FALSE)
	{
		$fullDir = self::_find_view_folder().$dir;
		if(file_exists($fullDir."/header.php")){
			$this->set_partial("header", $dir."/header", $data);
		}
		if(file_exists($fullDir."/footer.php")){
			$this->set_partial("footer", $dir."/footer", $data);
		}
		$view = $dir."/".$view;
		...
		if ($this->_layout)
		{
			...
			$this->_body =  self::_load_view('_layouts/'.$this->_layout, $this->_data, TRUE, self::_find_view_folder());
		}
	...
	public function get_layouts()
	{
		...
		foreach(glob(self::_find_view_folder().'_layouts/*.*') as $layout)
		{
	...
	public function get_theme_layouts($theme = NULL)
	{
		...
		foreach ($this->_theme_locations as $location)
		{
			...
			if( is_dir($location.$theme.'/views/web/_layouts/') )
			{
				foreach(glob($location.$theme . '/views/web/_layouts/*.*') as $layout)
				{
					$layouts[] = pathinfo($layout, PATHINFO_BASENAME);
				}
				break;
			...
			if(is_dir($location.$theme.'/views/_layouts/'))
			{
				foreach(glob($location.$theme . '/views/_layouts/*.*') as $layout)
				{
					$layouts[] = pathinfo($layout, PATHINFO_BASENAME);
				}
				break;
	...
	public function layout_exists($layout)
	{
		...
		return file_exists(self::_find_view_folder().'_layouts/' . $layout . self::_ext($layout));
	}
